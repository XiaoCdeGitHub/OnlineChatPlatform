<template>
  <div>
    <div class="login-box-outer">
      ...
      <div class="word-link" @click="changeMode" v-if="isLogin">立即注册</div>
      <div class="login-box-buttom" v-if="!isLogin">
      ...
      <div class="word-link" @click="changeMode" v-if="!isLogin">返回登录</div>
    </div>
  </div>
</template>
...
export default {
  created() {
    // console.log(sessionStorage.getItem('loginRedirectMessage'), 'sessionStorage.getItem')
    // if (sessionStorage.getItem('loginRedirectMessage')) {
    //   console.log('没登陆呢')
  ...
  mounted() {
    if (sessionStorage.getItem('loginRedirectMessage')) {
      this.$message.error(sessionStorage.getItem('loginRedirectMessage'));
      sessionStorage.removeItem('loginRedirectMessage');
    }
    ...
    toLogin() {
      ...
      this.$api.loginApi.toLogin({ email, password }).then(
        (res) => {
          ...
          if (res?.length == 0) {
            this.$message.error("账号或密码好像有点问题哦~");
          } else {
            this.$message({
              message: "登陆成功",
              type: "success",
