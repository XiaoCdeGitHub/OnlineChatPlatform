<template>
  <div class="chatlist-box">
    ...
    <el-empty v-if="!userList.length" description="请先添加好友哦">
      <template #default>
        <el-button @click="addFriendsClick">To Add</el-button>
      </template>
    </el-empty>

    <div class="message-box" v-if="userList.length">
      <div class="message-item" v-for="(item,index) in showList" :key="index" @click="selectChatUser(item)">
        <img :src="item.avatar" alt="" />
        <div class="name-box">{{ item.name }}</div>
        <div class="time-box">2024-7-2</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    // console.log('我发送了谁？', JSON.parse(sessionStorage.getItem("currentUser")).userId)
    this.$api.userApi
      .getAllFriends(JSON.parse(sessionStorage.getItem("currentUser")).userId)
      .then((res) => {
        res.forEach((e) => {
  ...
  methods: {
    selectChatUser(e) {
      ...
      // console.log(e,"this.$store.state.chat.chatUser.userId")
      sessionStorage.setItem("chatPerson",JSON.stringify(e))
    },
    addFriendsClick() {
