...
export default {
  mounted() {
    ...
    console.log(sessionStorage.getItem("currentUser"),"sessionStorageUser")
    this.userMessage = JSON.parse(sessionStorage.getItem("currentUser"))
    console.log('userMessage.avatar', this.userMessage.avatar)
  },
  methods: {
    updateAvatar() {
      ...
      if (this.isRevise) {
        ...
        this.$api.userApi.updateUserInfo(this.userMessage).then((res) => {
          if (res == "success") {
            ...
            console.log("修改成功", this.userMessage)
            sessionStorage.setItem("currentUser",JSON.stringify(this.userMessage))
          }
        });
      } else {
        // 修改信息
